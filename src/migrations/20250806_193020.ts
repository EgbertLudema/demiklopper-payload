...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`pages_blocks_form_block\` (
  	...
  	\`intro_content\` text,
  	\`enable_info\` integer,
  	\`info_block_content\` text,
  	\`info_block_location\` text,
  	\`info_block_instagram\` text,
  	\`block_name\` text,
  ...
  await db.run(sql`CREATE TABLE \`pages_blocks_portfolio_gallery\` (
  	...
  	\`replace_load_more_with_link\` integer,
  	\`block_name\` text,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`pages\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  ...
  await db.run(sql`CREATE INDEX \`pages_blocks_about_me_image_idx\` ON \`pages_blocks_about_me\` (\`image_id\`);`)
  await db.run(sql`CREATE TABLE \`pages_blocks_text_and_image\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`pages_blocks_text_and_image_order_idx\` ON \`pages_blocks_text_and_image\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`pages_blocks_text_and_image_parent_id_idx\` ON \`pages_blocks_text_and_image\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`pages_blocks_text_and_image_path_idx\` ON \`pages_blocks_text_and_image\` (\`_path\`);`)
  await db.run(sql`CREATE INDEX \`pages_blocks_text_and_image_media_idx\` ON \`pages_blocks_text_and_image\` (\`media_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`pages_rels_portfolio_id_idx\` ON \`pages_rels\` (\`portfolio_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`_pages_v_blocks_about_me_image_idx\` ON \`_pages_v_blocks_about_me\` (\`image_id\`);`)
  await db.run(sql`CREATE TABLE \`_pages_v_blocks_text_and_image\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_pages_v_blocks_text_and_image_order_idx\` ON \`_pages_v_blocks_text_and_image\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_pages_v_blocks_text_and_image_parent_id_idx\` ON \`_pages_v_blocks_text_and_image\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_pages_v_blocks_text_and_image_path_idx\` ON \`_pages_v_blocks_text_and_image\` (\`_path\`);`)
  await db.run(sql`CREATE INDEX \`_pages_v_blocks_text_and_image_media_idx\` ON \`_pages_v_blocks_text_and_image\` (\`media_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`_pages_v_rels_portfolio_id_idx\` ON \`_pages_v_rels\` (\`portfolio_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`portfolio_slug_idx\` ON \`portfolio\` (\`slug\`);`)
  ...
  await db.run(sql`CREATE INDEX \`portfolio_rels_portfolio_categories_id_idx\` ON \`portfolio_rels\` (\`portfolio_categories_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`_portfolio_v_version_version_image_idx\` ON \`_portfolio_v\` (\`version_image_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_portfolio_v_rels\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`order\` integer,
  	\`parent_id\` integer NOT NULL,
  	\`path\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_portfolio_v_rels_order_idx\` ON \`_portfolio_v_rels\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`_portfolio_v_rels_parent_idx\` ON \`_portfolio_v_rels\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_portfolio_v_rels_path_idx\` ON \`_portfolio_v_rels\` (\`path\`);`)
  await db.run(sql`CREATE INDEX \`_portfolio_v_rels_portfolio_categories_id_idx\` ON \`_portfolio_v_rels\` (\`portfolio_categories_id\`);`)
  await db.run(sql`CREATE TABLE \`portfolio_categories\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
  	\`slug\` text NOT NULL,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`portfolio_categories_slug_idx\` ON \`portfolio_categories\` (\`slug\`);`)
  await db.run(sql`CREATE INDEX \`portfolio_categories_updated_at_idx\` ON \`portfolio_categories\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`portfolio_categories_created_at_idx\` ON \`portfolio_categories\` (\`created_at\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`pages_blocks_text_and_image\`;`)
  ...
  await db.run(sql`DROP TABLE \`_pages_v_blocks_text_and_image\`;`)
  ...
  await db.run(sql`DROP TABLE \`_portfolio_v_rels\`;`)
  await db.run(sql`DROP TABLE \`portfolio_categories\`;`)
